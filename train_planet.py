...
parser.add_argument('--datafile_suffix', default=None, type=str, help="Suffix for path to train data fps (default is None)")
...
parser.add_argument('--adaptor', default="linear", type=str, help='Type of adaptor to convert 4-chan to 3-channels. choices: [linear, drop, no_init]')
...
parser.add_argument('--out', default='./results/planet-water', help='Directory to output the result')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    lgr.debug(f"Using num_inp_feats={num_inp_feats}")
    if opt.pretrained == 0:
        num_inp_feats = 3   # number of channels in input
        lgr.warning(f"Using model with random weights. opt.pretrained: {opt.pretrained}. num_inp_feats: {num_inp_feats}")
    ...
    train_dataset1 = PlanetSegmentation(root=opt.data_dir, split="train", resize_size=resize_size, adaptor=opt.adaptor)
    val_dataset1 = PlanetSegmentation(root=opt.data_dir, split="valid", resize_size=resize_size, adaptor=opt.adaptor)
